    ...
    async def registrar_actividad(self):
        ...
        try:
            async for mensaje in canal.history(limit=None, oldest_first=False):
                ...
                uid = str(mensaje.author.id)
                if uid not in ultimos_mensajes or mensaje.created_at > ultimos_mensajes[uid]:
                    ultimos_mensajes[uid] = mensaje.created_at
            ...
            await log_discord(self.bot, f"❌ [INACTIVIDAD] Error cargando historial: {e}", "error", "Inactividad")
                ...
                if ban_fecha_iso:
                    ...
                    if (ahora - ban_fecha).days >= DURACION_BANEO_DIAS:
                        try:
                            ...
                            await log_discord(self.bot, f"❌ [INACTIVIDAD] Error al desbanear a {user}: {e}", "error", "Inactividad")
